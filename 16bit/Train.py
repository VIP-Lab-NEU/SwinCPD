...
class PolarizationDatasetVal(Dataset):
    def __init__(self, lr_base_path, hr_base_path, split, scale_factor):
        ...
        self.scene_ids_from_dir = self._get_scene_ids() # Use same method
...
def save_validation_image(tensor_12ch, filename, output_dir):
    ...
    try:
        ...
        img_np_16bit = (torch.clamp(img_tensor_rgb0, 0, 1).permute(1, 2, 0).numpy() * 65535.0).astype(np.uint16)
        cv2.imwrite(os.path.join(output_dir, filename), cv2.cvtColor(img_np_16bit, cv2.COLOR_RGB2BGR))
