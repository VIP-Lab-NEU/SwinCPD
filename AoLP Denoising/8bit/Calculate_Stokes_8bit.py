...
def process_polarization_scene(folder_path: str, base_dir: str = "."):
    """
    Reads four 8-bit RGB polarization images (0, 45, 90, 135 degrees),
    calculates key polarization parameters (Stokes, DoLP, AoLP), and saves
    them as 8-bit PNG files.
    ...
    for angle in angles:
        ...
        if img_bgr.dtype != np.uint8:
            print(f"Warning: Image {img_path} is not 8-bit. It will be converted.")
    ...
    S0_save = S0 / 2.0
    save_8bit_image("S0.png", S0_save)

    S1_save = (S1 + 255.0) / 2.0
    S2_save = (S2 + 255.0) / 2.0
    save_8bit_image("S1.png", S1_save)
    save_8bit_image("S2.png", S2_save)

    P1_save = (P1 + 1.0) * 0.5 * 255.0
    P2_save = (P2 + 1.0) * 0.5 * 255.0
    save_8bit_image("P1.png", P1_save)
    save_8bit_image("P2.png", P2_save)

    DoLP_save = DoLP * 255.0
    save_8bit_image("DoLP.png", DoLP_save)

    AoLP_save = ((AoLP + np.pi / 2) / np.pi) * 255.0
    save_8bit_image("AoLP.png", AoLP_save)
    ...
    # Scale the BGR float image [0, 1] back to the 8-bit integer range [0, 255]
    bgr_image_8bit = bgr_image_float * 255.0

    # Save the final 8-bit color image
    save_8bit_image("AoLP_color.png", bgr_image_8bit)
